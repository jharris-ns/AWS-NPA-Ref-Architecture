...
1. **Pre-bake AMI with SSM agent** (if using custom AMI)
2. **Increase Lambda memory** (faster CPU allocation)
3. **Use smaller instance type initially** (faster boot)
4. **Reduce wait times** (if environment is consistent):

```python
# Conservative (default)
wait_times = [5, 10, 15, 20, 30, 30, 30, 30, 30, 30]  # ~4 min

# Aggressive (risky)
wait_times = [5, 10, 15, 15, 15]  # ~1 min
```
...
FunctionName: <PublisherGroupName>-RegistrationHandler
Threshold: >= 1
Period: 300 seconds
```
...
Namespace: AWS/Lambda
FunctionName: <PublisherGroupName>-RegistrationHandler
Threshold: >= 750000  # 12.5 minutes (out of 15)
```

**SSM Command Failures:**
```yaml
MetricName: CommandsFailedCount
Namespace: AWS/SSM
Threshold: >= 1
```
...
# In code:
put_metric('RegistrationTime', elapsed_seconds, 'Seconds')
put_metric('RegistrationSuccess', 1, 'Count')
```
...
Current: Re-running CREATE creates duplicate publishers

**Improvement:**
```python
def handle_create_idempotent(event, context):
    instance_id = event['ResourceProperties']['InstanceId']

    # Check if publisher already exists for this instance
    publishers = call_netskope_api('GET', 'infrastructure/publishers')
    for pub in publishers['data']:
        if instance_id in pub['publisher_name']:
            # Already exists, return existing ID
            return {'publisher_id': pub['id']}
...
Current: Lambda waits synchronously for command completion

**Improvement:** Use Step Functions:
```yaml
StateMachine:
  CreatePublisher -> WaitForEC2 -> WaitForSSM -> SendCommand ->
  WaitForCompletion -> UpdateApps -> NotifyCloudFormation
```
...
class PublisherRegistrationError(Exception):
    def __init__(self, phase, details):
        self.phase = phase  # 'ec2_wait', 'ssm_wait', 'command_exec'
        self.details = details
        self.troubleshooting = self.get_troubleshooting_steps()
    ...
    def get_troubleshooting_steps(self):
        steps = {
            'ssm_wait': [
                ...
                'Check IAM instance profile has SSM policy'
            ],
            # ...
        }
